...
class StatusDailyDaoTest extends AnyWordSpec with Matchers with BeforeAndAfter {
  ...
  "StatusDailyQueries replaceHours" should {
    "insert records into an empty table" in {
      ...
      val terminal = T2
      val statusDaily =
        StatusDaily(portCode, terminal, LocalDate(2020, 1, 1), Option(1), Option(1), Option(1), Option(1))
    ...
    "only insert/update entries for the port and terminal specified" in {
      ...
      maybeStatus should
        be(Some(StatusDaily(portCode, terminal, LocalDate(2020, 1, 1), Option(3), Option(3), Option(3), Option(3))))
    }
