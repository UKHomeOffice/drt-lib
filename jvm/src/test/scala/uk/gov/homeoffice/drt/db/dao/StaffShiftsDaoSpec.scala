...
class StaffShiftsDaoSpec extends Specification with BeforeEach {
  ...
  "StaffShiftsDao" should {
    ...
    "latestStaffShiftForADate should return the correct shift for startDate 01-07-2025" in {
      ...
      val shiftName = "Early"
      val currentDate = LocalDate(2025, 7, 1)
      val staffShift = getStaffShiftRow.copy(
        port = port,
        terminal = terminal,
        shiftName = shiftName,
    ...
    "createNewShiftWhileEditing should set endDate to a day before new startDate if previous shift has no endDate and spans today" in {
      ...
      val previousResult = Await.result(
        dao.searchStaffShift(prevShift.port, prevShift.terminal, prevShift.shiftName, prevShift.startDate),
        1.second
      )
      val result: Shift = Await.result(dao.createNewShiftWhileEditing(prevShift, newShift), 1.second)
      val updatedPrev = Await.result(
        dao.searchStaffShift(prevShift.port, prevShift.terminal, prevShift.shiftName, prevShift.startDate),
        1.second
      )
    ...
    "createNewShiftWhileEditing should update previous shift endDate to start date minus one day of new shift and also latestStaffShiftForADate for different shiftDates" in {
      ...
      val newShiftRequestedWithOctStartDate =
        getStaffShiftRow.copy(startDate = LocalDate(2025, 10, 1), endDate = None, shiftName = newShiftName)
      Await.result(dao.insertOrUpdate(prevShiftInAug), 1.second)
      val updatedShiftsResults =
        Await.result(dao.createNewShiftWhileEditing(prevShiftInAug, newShiftRequestedWithOctStartDate), 1.second)
      val prevShiftInAugResultAfterUpdates = Await.result(
        dao.latestStaffShiftForADate(
          prevShiftInAug.port,
          prevShiftInAug.terminal,
          prevShiftInAug.startDate,
          prevShiftInAug.startTime
        ),
        1.second
      )
      val newShiftResultWithOctStartDate = Await.result(
        dao.latestStaffShiftForADate(
          newShiftRequestedWithOctStartDate.port,
          newShiftRequestedWithOctStartDate.terminal,
          newShiftRequestedWithOctStartDate.startDate,
          newShiftRequestedWithOctStartDate.startTime
        ),
        1.second
      )

      val searchShiftForStartDateInSep = getStaffShiftRow.copy(startDate = LocalDate(2025, 9, 1))
      val searchResultForStartDateInSep = Await.result(
        dao.latestStaffShiftForADate(
          searchShiftForStartDateInSep.port,
          searchShiftForStartDateInSep.terminal,
          searchShiftForStartDateInSep.startDate,
          searchShiftForStartDateInSep.startTime
        ),
        1.second
      )
      val searchShiftForStartDateInNov = getStaffShiftRow.copy(startDate = LocalDate(2025, 11, 1))
      val searchResultForStartDateInNov = Await.result(
        dao.latestStaffShiftForADate(
          searchShiftForStartDateInNov.port,
          searchShiftForStartDateInNov.terminal,
          searchShiftForStartDateInNov.startDate,
          searchShiftForStartDateInNov.startTime
        ),
        1.second
      )
      ...
      newShiftResultWithOctStartDate.get ===
        newShiftRequestedWithOctStartDate.copy(createdAt = newShiftResultWithOctStartDate.get.createdAt)
    ...
    "latestStaffShiftForADate for startDate in the future with start time change to 1 hour ahead" in {
      ...
      Await.result(dao.insertOrUpdate(prevShiftInAug), 1.second)
      val newShiftName = "OctoberShift"
      val newShiftRequestedWithOctStartDate = getStaffShiftRow.copy(
      ...
      val searchResultForStartDateInOct = Await.result(
        dao.latestStaffShiftForADate(
          searchShiftForStartDateInOct.port,
          searchShiftForStartDateInOct.terminal,
          searchShiftForStartDateInOct.startDate,
          searchShiftForStartDateInOct.startTime
        ),
        1.second
      )

      updatedShiftsResults.shiftName === newShiftName
      searchResultForStartDateInOct.get ===
        newShiftRequestedWithOctStartDate.copy(createdAt = searchResultForStartDateInOct.get.createdAt)
    }
    ...
    "updateStaffShift should update previous shift end date and remove future shifts and insert the new shift" in {
      val previousShift = getStaffShiftRow.copy(
        port = "LHR",
        terminal = "T1",
        shiftName = "Early-Aug",
        startDate = LocalDate(2024, 8, 1),
        startTime = "08:00",
        endDate = Option(LocalDate(2024, 9, 30)),
        createdAt = 0L
      )
      val futureExistingShift = getStaffShiftRow.copy(
        port = "LHR",
        terminal = "T1",
        shiftName = "Early-Oct",
      ...
      val shiftRow = getStaffShiftRow.copy(
        port = "LHR",
        terminal = "T1",
        shiftName = "Early-Sep",
      ...
      val updatedShiftsResults =
        Await.result(dao.updateStaffShift(previousShift, futureExistingShift, shiftRow), 1.second)
      ...
      val updatedPreviousShift = Await.result(
        dao.searchStaffShift(
          previousShift.port,
      ...
      val updatedFutureShift = Await.result(
        dao.searchStaffShift(
          futureExistingShift.port,
          futureExistingShift.terminal,
          futureExistingShift.shiftName,
          futureExistingShift.startDate
        ),
        1.second
      )
