...
class MockPredictionModelActor(probeRef: ActorRef) extends PredictionModelActor(
      () => SDate.now(),
      FlightCategory,
      TerminalFlightNumberOrigin("T1", 100, "JFK"),
      None
    ) {
  override def persistAndMaybeSnapshotWithAck(
...
class PredictionModelActorTest extends TestKit(ActorSystem("Predictions"))
  ...
  "A PredictionModel actor" should {
    ...
    val modelUpdate = ModelUpdate(
      RegressionModel(Seq(1, 2), 1.4),
      OffScheduleModelAndFeatures.featuresVersion,
      features,
      10,
      10.1,
