...
class ArrivalsDiffSpec extends Specification {
  ...
  "When I apply ArrivalsDiff to FlightsWithSplits" >> {
    val arrival =
      ArrivalGenerator.arrival(iata = "BA0001", status = ArrivalStatus("new status"), feedSource = AclFeedSource)
    ...
    arrivalsDiff.applyTo(existing, now, List(AclFeedSource))._1 ===
      FlightsWithSplits(Map(arrival.unique -> ApiFlightWithSplits(updatedArrival, Set(), Option(10L))))
  }
