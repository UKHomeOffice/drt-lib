...
class BorderCrossingDaoTest extends AnyWordSpec with Matchers with BeforeAndAfter {
  ...
  "totalForPortAndDate" should {
    ...
    "return the hourly passengers for a port, terminal and local date (spanning 2 utc dates)" in {
      ...
      val resultT2: Map[Long, Map[Queues.Queue, Int]] = TestDatabase.run(dao.hourlyForPortAndDate(
        portCode.iata,
        Option(T2.toString)
      )(global)(LocalDate(2023, 6, 10))).futureValue
  ...
  "removeAllBefore" should {
    "only remove rows with a date earlier than the data specified" in {
      ...
      Seq(
        ...
        case (date, expected) =>
          val rows =
            TestDatabase.run(dao.get(portCode.iata, terminal.toString, UtcDate(2020, 1, date).toISOString)).futureValue
  ...
  "filterDate" should {
    ...
    "filter rows by utc date" in {
      ...
      val rows =
        TestDatabase.run(dao.get(portCode.iata, terminal.toString, UtcDate(2025, 5, 2).toISOString)).futureValue
      rows.toSet.map(BorderCrossingSerialiser.fromRow) should
        be(Set(BorderCrossing(portCode, terminal, UtcDate(2025, 5, 2), Pcp, 2, 2)))
    }
  ...
  "queueTotalsForPortAndDate" should {
    "return the total pax for a port and local date that has pax spanning 2 utc dates" in {
      (SDate("2025-05-01T20:00").millisSinceEpoch to SDate("2025-05-03T20:00").millisSinceEpoch by
        1.hour.toMillis).map { millis =>
      ...
      val result =
        TestDatabase.run(dao.queueTotalsForPortAndDate(portCode.iata, None)(global)(LocalDate(2025, 5, 2))).futureValue
    ...
    "return the total pax for a port and utc date" in {
      ...
      val result =
        TestDatabase.run(dao.queueTotalsForPortAndDate(portCode.iata, None)(global)(UtcDate(2025, 5, 2))).futureValue
