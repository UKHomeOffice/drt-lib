...
trait FlightsWithSplitsExport {
  ...
  val standardFilter: (ApiFlightWithSplits, Seq[Terminal]) => Boolean =
    (fws, terminals) => terminals.contains(fws.apiFlight.Terminal)
  ...
  private def flightToCsvRow(fws: ApiFlightWithSplits, maybeManifest: Option[VoyageManifest]): String =
    rowValues(fws, maybeManifest).mkString(",")

  def prepend: Option[String] = Option(headings)

  def csvStream(flightsStream: Source[(Iterable[ApiFlightWithSplits], VoyageManifests), NotUsed])
