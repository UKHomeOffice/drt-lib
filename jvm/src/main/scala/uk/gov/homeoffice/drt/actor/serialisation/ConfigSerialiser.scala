...
trait ConfigSerialiser[B, A <: Configs[B]] {
  ...
  def removeUpdate(a: RemoveConfig, createdAt: Long): RemoveConfigMessage =
    RemoveConfigMessage(Option(a.effectiveFrom), Option(createdAt))
}

object ConfigSerialiser {
  implicit val slaConfigsSerialiser: ConfigSerialiser[Map[Queue, Int], SlaConfigs] =
    new ConfigSerialiser[Map[Queue, Int], SlaConfigs] {
      ...
      override def setUpdate(a: SetUpdate[Map[Queue, Int]], createdAt: Long): GeneratedMessage =
        setSlasUpdatesToMessage(a, createdAt)
    }
}
