...
case class PassengersHourlyRow(
    portCode: String,
    terminal: String,
    queue: String,
    dateUtc: String,
...
class PassengersHourlyTable(tag: Tag)
  ...
  def portTerminalDateHourIndex =
    index("idx_passengers_hourly_port_terminal_date_hour", (port, terminal, dateUtc, hour), unique = false)

  def portTerminalDateIndex =
    index("idx_passengers_hourly_port_terminal_date", (port, terminal, dateUtc), unique = false)
  ...
  def * =
    (
      ...
      passengers,
      updatedAt
    ) <> (PassengersHourlyRow.tupled, PassengersHourlyRow.unapply)
}
