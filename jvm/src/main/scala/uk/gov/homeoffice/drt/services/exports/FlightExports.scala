...
object FlightExports {
  ...
  def splitsForSources(
      fws: ApiFlightWithSplits,
      paxFeedSourceOrder: List[FeedSource]
  ): List[String] =
    splitSources.flatMap((ss: SplitSource) =>
      queueSplits(ArrivalExportHeadings.queueNamesInOrder, fws, ss, paxFeedSourceOrder)
    )

  private def queueSplits(
      queueNames: Seq[Queue],
      fws: ApiFlightWithSplits,
  ...
  private def queuePaxForFlightUsingSplits(
      fws: ApiFlightWithSplits,
      splitSource: SplitSource,
      paxFeedSourceOrder: List[FeedSource]
  ): Map[Queue, Int] =
  ...
  def actualAPISplitsAndHeadingsFromFlight(flightWithSplits: ApiFlightWithSplits): Set[(String, Double)] =
    flightWithSplits
      .splits
      .collect {
  ...
  def actualAPISplitsForFlightInHeadingOrder(
