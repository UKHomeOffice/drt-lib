...
trait RecoveryLogging {
  ...
  def snapshotOfferLogMessage(md: SnapshotMetadata): String =
    s"$persistenceId $prefix received SnapshotOffer from ${SDate(md.timestamp).toISOString}, sequence number ${md.sequenceNr}"

  def logSnapshotOffer(md: SnapshotMetadata): Unit = log.debug(snapshotOfferLogMessage(md))

  def logSnapshotOffer(
      md: SnapshotMetadata,
      additionalInfo: String
  ): Unit = log.debug(s"${snapshotOfferLogMessage(md)} - $additionalInfo")
  ...
  def logCounters(bytes: Int, messages: Int, bytesThreshold: Int, maybeMessageThreshold: Option[Int]): Unit = {
    ...
    log.debug(
      f"$megaBytes%.2fMB persisted in $messages messages since last snapshot. Thresholds: $megaBytesThreshold%.2fMB, $maybeMessageThreshold messages"
    )
  }
}
