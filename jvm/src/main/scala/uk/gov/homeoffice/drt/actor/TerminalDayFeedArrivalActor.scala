...
object TerminalDayFeedArrivalActor {
  ...
  private def forecast(processRemovals: Boolean)(
    ...
    Props(new TerminalDayFeedArrivalActor(
      year,
      month,
      day,
      terminal,
  ...
  def props(
    ...
    if (feedSource == AclFeedSource)
      forecast(processRemovals =
        true
      )(year, month, day, terminal, feedSource, maybePointInTime, now, maxSnapshotInterval)
...
class TerminalDayFeedArrivalActor[A <: FeedArrival](
    year: Int,
    month: Int,
    day: Int,
    terminal: Terminal,
  ...
  override def persistenceId: String =
    f"${feedSource.id}-feed-arrivals-${terminal.toString.toLowerCase}-$year-$month%02d-$day%02d"
