...
trait IShiftMetaInfoDaoLike {
  ...
  def updateShiftAssignmentsMigratedAt(
      port: String,
      terminal: String,
      shiftAssignmentsMigratedAt: Option[Long]
  )(implicit ex: ExecutionContext): Future[Option[ShiftMeta]]
...
case class ShiftMetaInfoDao(db: CentralDatabase) extends IShiftMetaInfoDaoLike {
  ...
  def insertShiftMetaInfo(shiftMeta: ShiftMeta)(implicit ex: ExecutionContext): Future[Int] = {
    val query = shiftMetaInfoTable += ShiftMetaInfoRow(
      shiftMeta.port,
      shiftMeta.terminal,
      shiftMeta.shiftAssignmentsMigratedAt.map(new java.sql.Timestamp(_))
    )
    db.db.run(query)
  }

  override def getShiftMetaInfo(port: String, terminal: String)(implicit
      ex: ExecutionContext
  ...
  private def shiftMetaData(action: SqlAction[Option[ShiftMetaInfoRow], NoStream, Effect.Read])(implicit
      ex: ExecutionContext
  ) = {
  ...
  override def updateShiftAssignmentsMigratedAt(
    ...
    val action =
      query.andThen(shiftMetaInfoTable.filter(row => row.port === port && row.terminal === terminal).result.headOption)
    actionShiftMetaData(action)
  }

  private def actionShiftMetaData(action: DBIOAction[
    Option[ShiftMetaInfoRow],
    NoStream,
