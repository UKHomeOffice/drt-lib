...
trait HttpClient {
  ...
  def httpRequestForPortCsv(uri: String, portCode: PortCode): HttpRequest = {
    val roleHeaders = rolesToRoleHeader(List(
      Option(Roles.ArrivalsAndSplitsView),
      Option(Roles.ApiView),
      Roles.parse(portCode.iata)
    ).flatten)
    HttpRequest(
...
case class ProdHttpClient(sendHttpRequest: HttpRequest => Future[HttpResponse])(implicit
