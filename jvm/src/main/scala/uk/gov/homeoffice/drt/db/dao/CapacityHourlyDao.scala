...
object CapacityHourlyDao {
  ...
  def replaceHours(port: PortCode)
      : (Terminal, Iterable[CapacityHourlyRow]) => DBIOAction[Unit, NoStream, Effect.Write with Effect.Transactional] =
    (terminal, rows) => {
  ...
  def hourlyForPortAndDate(port: String, maybeTerminal: Option[String])(implicit
      ec: ExecutionContext
  ): LocalDate => DBIOAction[Map[Long, Int], NoStream, Effect.Read] =
    localDate =>
      filterPortTerminalDate(port, maybeTerminal, localDate)
        .map {
          _
  ...
  private def filterPortTerminalDate(port: String, maybeTerminal: Option[String], localDate: LocalDate)(implicit
      ec: ExecutionContext
  ): DBIOAction[Seq[CapacityHourlyRow], NoStream, Effect.Read] = {
