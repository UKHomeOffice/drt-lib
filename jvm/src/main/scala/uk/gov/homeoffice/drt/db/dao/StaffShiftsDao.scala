...
case class StaffShiftsDao(db: CentralDatabase)(implicit ec: ExecutionContext) extends IStaffShiftsDao {
  ...
  override def getStaffShiftsByPortAndTerminal(port: String, terminal: String): Future[Seq[Shift]] =
    db.run(
      staffShiftsTable.filter(s => s.port === port && s.terminal === terminal).sortBy(_.startDate.desc).result
  ...
  override def getStaffShift(
    ...
    db.run(staffShiftsTable.filter(s =>
      s.port === port &&
        s.terminal === terminal &&
  ...
  override def latestStaffShiftForADate(
    ...
    db.run(staffShiftsTable.filter(s =>
      s.port === port && s.terminal === terminal &&
        (s.startDate === startDateSql ||
          s.startDate <= startDateSql && (s.endDate >= startDateSql || s.endDate.isEmpty)) &&
  ...
  override def updateStaffShift(previousShift: Shift, futureExistingShift: Shift, shiftRow: Shift): Future[Shift] = {
    ...
    val endDateADayBefore = new java.sql.Date(staffShiftRow.startDate.getTime - 24L * 60 * 60 * 1000)
    val updatePreviousExitingStaffShiftRow = if (previousStaffShiftRow.endDate.isDefined)
      previousStaffShiftRow.copy(endDate = Option(endDateADayBefore))
    else previousStaffShiftRow.copy(endDate = None)
    val insertPreviousExitingAction = staffShiftsTable += updatePreviousExitingStaffShiftRow

    val updateFutureExitingStaffShiftRow = if (futureExistingShiftRow.endDate.isDefined)
      staffShiftRow.copy(endDate = futureExistingShiftRow.endDate)
    else staffShiftRow.copy(endDate = None)
