...
object CrunchRequestMessageConversion {
  ...
  def removeProcessingRequestToMessage(request: RemoveProcessingRequest): GeneratedMessage = {
    ...
    RemoveCrunchRequestMessage(
      ...
      Option(date.month),
      Option(date.day),
      Option(request.request.terminal.toString)
    )
  }
  ...
  def terminalUpdateRequestsFromMessage(terminals: LocalDate => Iterable[Terminal])
      : CrunchRequestMessage => Seq[TerminalUpdateRequest] = {
