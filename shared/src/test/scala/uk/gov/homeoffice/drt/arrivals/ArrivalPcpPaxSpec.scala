    ...
    "Given an arrival with 100 pax from API and 50 from Act Pax " +
      "Then I should expect 50 PCP pax" >> {
        val a = arrival(
          ...
          passengerSources =
            Map(LiveFeedSource -> Passengers(Option(50), None), ApiFeedSource -> Passengers(Option(100), None))
        )
    ...
    "Given an arrival with 100 act pax and 10 Transfer" +
      "Then I should expect 90 PCP pax" >> {
        val a = arrival(
          feedSources = Set(LiveFeedSource),
          passengerSources =
            Map(LiveFeedSource -> Passengers(Option(100), Option(10)), ApiFeedSource -> Passengers(Option(100), None))
        )
        val result = a.bestPcpPaxEstimate(sourceOrderPreference).getOrElse(0)
        val expected = 90
    ...
    "Given an arrival with 100 pax from API and 50 from Act Pax " +
      "Then I should expect 100 PCP pax - API trumps ACL numbers" >> {
        val a = arrival(
          ...
          passengerSources =
            Map(ApiFeedSource -> Passengers(Option(100), None), AclFeedSource -> Passengers(Option(50), None))
        )
