  ...
  "When I diff SplitsForArrivals with a FlightsWithSplits" >> {
    "Given one new split and no existing ones" >> {
      "Then I should get a SplitsForArrivals containing the new split" >> {
        val splitsForArrivals =
          SplitsForArrivals(Map(uniqueArrival -> Set(Splits(Set(), Historical, None, PaxNumbers))))
    ...
    "Given one new split and one existing split with the same values as the new one" >> {
      "Then I should get an empty diff" >> {
        val existingSplits = Set(Splits(
          Set(ApiPaxTypeAndQueueCount(EeaMachineReadable, EGate, 1, None, None)),
          Historical,
          None,
          PaxNumbers
        ))
    ...
    "Given one new split and one existing split of the same source but new numbers" >> {
      "Then I should get a SplitsForArrivals containing updated split" >> {
        ...
        val newSplits = Set(Splits(
          Set(ApiPaxTypeAndQueueCount(EeaNonMachineReadable, EeaDesk, 1, None, None)),
          Historical,
          None,
          PaxNumbers
        ))
  ...
  "When I apply SplitsForArrivals to FlightsWithSplits" >> {
    "Given one new split and one matching arrival with 2 existing splits, one from the same source" >> {
      "Then I should get a FlightsWithSplits containing the matching arrival with the newly updated split along with the other pre-existing split" >> {
        val existingSplits1 =
          Splits(Set(ApiPaxTypeAndQueueCount(EeaMachineReadable, EGate, 1, None, None)), Historical, None, PaxNumbers)
        val existingSplits2 = Splits(
          Set(ApiPaxTypeAndQueueCount(VisaNational, EGate, 1, None, None)),
          ApiSplitsWithHistoricalEGateAndFTPercentages,
          None,
          PaxNumbers
        )
        val newSplits = Splits(
          Set(ApiPaxTypeAndQueueCount(EeaNonMachineReadable, EeaDesk, 1, None, None)),
          ApiSplitsWithHistoricalEGateAndFTPercentages,
          None,
          PaxNumbers
        ...
        val arrival = ArrivalGeneratorShared.arrival(
          iata = "BA0001",
          terminal = T1,
          origin = PortCode("JFK"),
          passengerSources =
            Map(LiveFeedSource -> Passengers(Option(1), Some(0)), ApiFeedSource -> Passengers(Option(1), None)),
        ...
        updated === FlightsWithSplits(Seq(ApiFlightWithSplits(
          arrival.copy(
            FeedSources = Set(ApiFeedSource, LiveFeedSource),
            PassengerSources =
              Map(ApiFeedSource -> Passengers(Option(1), Option(0)), LiveFeedSource -> Passengers(Option(1), Option(0)))
          ),
          Set(newSplits, existingSplits1),
          Option(now)
        )))
      }
    }
    ...
    "Given 2 splits and two arrivals, one with a new source and one with the same source" >> {
      "Then I should get a FlightsWithSplits containing the arrivals updated with the correct new splits" >> {
        val arrival1 = ArrivalGeneratorShared.arrival(
          iata = "BA0001",
          terminal = T1,
          origin = PortCode("JFK"),
          passengerSources = Map(LiveFeedSource -> Passengers(Option(1), Some(0)))
        )
        val arrival2 = ArrivalGeneratorShared.arrival(
          iata = "FR1234",
          terminal = T1,
          origin = PortCode("JFK"),
          passengerSources = Map(LiveFeedSource -> Passengers(Option(1), Some(0)))
        ...
        val existingSplits1 =
          Splits(Set(ApiPaxTypeAndQueueCount(VisaNational, NonEeaDesk, 1, None, None)), Historical, None, PaxNumbers)
        val existingSplits2 =
          Splits(Set(ApiPaxTypeAndQueueCount(EeaMachineReadable, EGate, 1, None, None)), Historical, None, PaxNumbers)
        val newSplits1 = Splits(
          Set(ApiPaxTypeAndQueueCount(EeaNonMachineReadable, EeaDesk, 1, None, None)),
          Historical,
          None,
          PaxNumbers
        )
        val newSplits2 = Splits(
          Set(ApiPaxTypeAndQueueCount(EeaNonMachineReadable, EeaDesk, 1, None, None)),
          ApiSplitsWithHistoricalEGateAndFTPercentages,
          None,
          PaxNumbers
        ...
        val splitsForArrivals =
          SplitsForArrivals(Map(arrival1.unique -> Set(newSplits1), arrival2.unique -> Set(newSplits2)))
