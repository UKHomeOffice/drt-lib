...
class ApiFlightWithSplitsSpec extends Specification {
  "A flight with splits" should {
    ...
    "have valid Api when api splits pax count is within the 5% Threshold of LiveSourceFeed pax count" in {
      "and there are no transfer pax" in {
        val flightWithSplits = flightWithPaxAndApiSplits(
          41,
          0,
          Set(LiveFeedSource),
          Map(LiveFeedSource -> Passengers(Option(40), Option(0))),
          scheduledAfterPaxSources
        )
        val apiSplits =
          flightWithSplits.splits.find(_.source == SplitSources.ApiSplitsWithHistoricalEGateAndFTPercentages).get
        apiSplits.isWithinThreshold(
          flightWithSplits.apiFlight.PassengerSources.get(LiveFeedSource),
          ApiFlightWithSplits.liveApiTolerance
        ) mustEqual true
      ...
      "and there are transfer pax only in the port feed data" in {
        val flightWithSplits = flightWithPaxAndApiSplits(
          ...
          0,
          Set(LiveFeedSource),
          Map(LiveFeedSource -> Passengers(Option(40), Option(20))),
          scheduledAfterPaxSources
        )
        val apiSplits =
          flightWithSplits.splits.find(_.source == SplitSources.ApiSplitsWithHistoricalEGateAndFTPercentages).get
      ...
      "and there are transfer pax both in the API data and in the port feed" in {
        ...
        val apiSplits =
          flightWithSplits.splits.find(s => s.source == SplitSources.ApiSplitsWithHistoricalEGateAndFTPercentages).get
        apiSplits.isWithinThreshold(
          flightWithSplits.apiFlight.PassengerSources.get(LiveFeedSource),
          ApiFlightWithSplits.liveApiTolerance
        ) mustEqual true
      ...
      "or there is a ScenarioSimulationSource" in {
        val flightWithSplits = flightWithPaxAndApiSplits(
          41,
          0,
          Set(LiveFeedSource, ScenarioSimulationSource),
          Map(LiveFeedSource -> Passengers(Option(20), Option(0))),
          scheduledAfterPaxSources
        )
        flightWithSplits.hasValidApi mustEqual true
      }
    ...
    "not have valid Api splits when the splits source type is not ApiSplitsWithHistoricalEGateAndFTPercentages" in {
      val flightWithSplits = flightWithPaxAndHistoricSplits(
        41,
        0,
        Set(LiveFeedSource),
        Map(LiveFeedSource -> Passengers(Option(40), Option(0)))
    ...
    "have valid Api splits when flight has a LiveFeedSource and a Live TotalPaxSource containing no passenger numbers" in {
      val flightWithSplits = flightWithPaxAndApiSplits(
        ...
        0,
        Set(LiveFeedSource),
        Map(LiveFeedSource -> Passengers(None, None)),
        scheduledAfterPaxSources
      )
      flightWithSplits.hasValidApi mustEqual true
    }
    ...
    "have no valid Api splits when flight scheduled before pax sources were recorded has a LiveFeedSource and ActPax more than 5% different to API pax" in {
      val flightWithSplits = flightWithPaxAndApiSplits(
        ...
        0,
        Set(LiveFeedSource),
        Map(LiveFeedSource -> Passengers(Option(100), Option(0))),
        scheduledBeforePaxSources
      )
      flightWithSplits.hasValidApi mustEqual false
    }
    ...
    "when there no actual pax number in liveFeed" in {
      "and api splits has pax number and hasValidApi is true" in {
        val flightWithSplits = flightWithPaxAndApiSplits(
          ...
          0,
          Set(LiveFeedSource),
          Map(LiveFeedSource -> Passengers(None, Option(0))),
          scheduledAfterPaxSources
        )
        flightWithSplits.hasValidApi mustEqual true
        flightWithSplits.apiFlight.bestPcpPaxEstimate(sourceOrderPreference) must beSome(100)
        val paxPerQueue: Option[Map[Queues.Queue, Int]] =
          ApiSplitsToSplitRatio.paxPerQueueUsingBestSplitsAsRatio(flightWithSplits, sourceOrderPreference)
    ...
    "when there actual pax number in liveFeed" in {
      "and api splits has pax number and hasValidApi is false" in {
        val flightWithSplits = flightWithPaxAndApiSplits(
          ...
          0,
          Set(LiveFeedSource),
          Map(LiveFeedSource -> Passengers(Option(95), Option(0))),
          scheduledAfterPaxSources
        )
        flightWithSplits.hasValidApi mustEqual false
  ...
  "bestSplits" should {
    val flight = ApiFlightWithSplits(
      apiFlight = ArrivalGeneratorShared.arrival(
        iata = "BA0001",
    ...
    val apiSplits = Splits(
      Set(ApiPaxTypeAndQueueCount(VisaNational, Queues.NonEeaDesk, 100, None, None)),
      SplitSources.ApiSplitsWithHistoricalEGateAndFTPercentages,
      Option(EventTypes.DC)
    )
    val invalidApiSplits = Splits(
      Set(ApiPaxTypeAndQueueCount(VisaNational, Queues.NonEeaDesk, 1, None, None)),
      SplitSources.ApiSplitsWithHistoricalEGateAndFTPercentages,
      Option(EventTypes.DC)
    )
    val validButAllTransitApi = Splits(
      Set(ApiPaxTypeAndQueueCount(Transit, Queues.Transfer, 100, None, None)),
      SplitSources.ApiSplitsWithHistoricalEGateAndFTPercentages,
      Option(EventTypes.DC)
    )
    val historicalSplits = Splits(
      Set(ApiPaxTypeAndQueueCount(VisaNational, Queues.NonEeaDesk, 100, None, None)),
      SplitSources.Historical,
      Option(EventTypes.DC)
    )
    val allTransitHistoricalSplits = Splits(
      Set(ApiPaxTypeAndQueueCount(Transit, Queues.Transfer, 100, None, None)),
      SplitSources.Historical,
      Option(EventTypes.DC)
    )
    val terminalSplits = Splits(
      Set(ApiPaxTypeAndQueueCount(VisaNational, Queues.NonEeaDesk, 100, None, None)),
  ...
  private def flightWithPaxAndApiSplits(
      splitsDirect: Int,
      splitsTransfer: Int,
    ...
    ApiFlightWithSplits(
      flight,
      Set(splitsForPax(
        directPax = splitsDirect,
        transferPax = splitsTransfer,
        ApiSplitsWithHistoricalEGateAndFTPercentages
      ))
    )
  }

  private def flightWithPaxAndHistoricSplits(
      splitsDirect: Int,
      splitsTransfer: Int,
      sources: Set[FeedSource],
      passengerSources: Map[FeedSource, Passengers]
  ...
  def splitsForPax(directPax: Int, transferPax: Int, source: SplitSource): Splits = Splits(
    Set(
