...
class ApiFlightWithSplitsSpec extends Specification {
  "A flight with splits" should {
    "have valid Api when api splits pax count is within the 5% Threshold of LiveSourceFeed pax count" in {
      "and there are no transfer pax" in {
        val flightWithSplits = flightWithPaxAndApiSplits(Option(40), 0, 41, 0, Set(LiveFeedSource),
          SortedSet(TotalPaxSource(40, LiveFeedSource, None)))
      ...
      "and there are transfer pax only in the port feed data" in {
        val flightWithSplits = flightWithPaxAndApiSplits(Option(40), 20, 21, 0, Set(LiveFeedSource), SortedSet(TotalPaxSource(40, LiveFeedSource, None)))
        val apiSplits = flightWithSplits.splits.find(_.source == SplitSources.ApiSplitsWithHistoricalEGateAndFTPercentages).get
        flightWithSplits.isWithinThreshold(apiSplits) mustEqual true
        flightWithSplits.hasValidApi mustEqual true
      }
      ...
      "and there are transfer pax only in the API data" in {
        val flightWithSplits = flightWithPaxAndApiSplits(Option(40), 0, 41, 20, Set(LiveFeedSource), SortedSet(TotalPaxSource(40, LiveFeedSource, None)))
        val apiSplits = flightWithSplits.splits.find(_.source == SplitSources.ApiSplitsWithHistoricalEGateAndFTPercentages).get
        flightWithSplits.isWithinThreshold(apiSplits) mustEqual true
        flightWithSplits.hasValidApi mustEqual true
      }
      ...
      "and there are transfer pax both in the API data and in the port feed" in {
        val flightWithSplits = flightWithPaxAndApiSplits(Option(40), 20, 21, 20, Set(LiveFeedSource), SortedSet(TotalPaxSource(40, LiveFeedSource, None)))
      ...
      "or there is a ScenarioSimulationSource" in {
        val flightWithSplits = flightWithPaxAndApiSplits(Option(20), 0, 41, 0, Set(LiveFeedSource, ScenarioSimulationSource), SortedSet())
    ...
    "not have valid Api when api splits pax count outside the 5% Threshold of LiveSourceFeed pax count" in {
      "and there are no transfer pax" in {
        val flightWithSplits = flightWithPaxAndApiSplits(Option(40), 0, 45, 0, Set(LiveFeedSource), SortedSet(TotalPaxSource(40, LiveFeedSource, None)))
        val apiSplits = flightWithSplits.splits.find(_.source == SplitSources.ApiSplitsWithHistoricalEGateAndFTPercentages).get
      ...
      "and there are transfer pax only in the port feed data" in {
        val flightWithSplits = flightWithPaxAndApiSplits(Option(40), 20, 24, 0, Set(LiveFeedSource), SortedSet(TotalPaxSource(40, LiveFeedSource, None)))
        val apiSplits = flightWithSplits.splits.find(_.source == SplitSources.ApiSplitsWithHistoricalEGateAndFTPercentages).get
        flightWithSplits.isWithinThreshold(apiSplits) mustEqual false
        flightWithSplits.hasValidApi mustEqual false
      }
      ...
      "and there are transfer pax only in the API data" in {
        val flightWithSplits = flightWithPaxAndApiSplits(Option(40), 0, 21, 25, Set(LiveFeedSource), SortedSet(TotalPaxSource(40, LiveFeedSource, None)))
        val apiSplits = flightWithSplits.splits.find(_.source == SplitSources.ApiSplitsWithHistoricalEGateAndFTPercentages).get
        flightWithSplits.isWithinThreshold(apiSplits) mustEqual false
        flightWithSplits.hasValidApi mustEqual false
      }
      ...
      "and there are transfer pax both in the API data and in the port feed" in {
        val flightWithSplits = flightWithPaxAndApiSplits(Option(40), 20, 25, 25, Set(LiveFeedSource), SortedSet(TotalPaxSource(40, LiveFeedSource, None)))
        val apiSplits = flightWithSplits.splits.find(s => s.source == SplitSources.ApiSplitsWithHistoricalEGateAndFTPercentages).get
        flightWithSplits.isWithinThreshold(apiSplits) mustEqual false
        flightWithSplits.hasValidApi mustEqual false
      }
    ...
    "have valid Api splits when flight is has no LiveFeedSource" in {
      "and pax count differences are within the threshold" in {
        val flightWithSplits = flightWithPaxAndApiSplits(Option(40), 0, 40, 0, Set(), SortedSet())
        flightWithSplits.hasValidApi mustEqual true
      }
      "and pax count differences are outside the threshold" in {
        val flightWithSplits = flightWithPaxAndApiSplits(Option(40), 0, 100, 0, Set(), SortedSet())
        flightWithSplits.hasValidApi mustEqual true
      }
    }
    ...
    "when there no actual pax number in liveFeed" in {
      "and api splits has pax number and hasValidApi is true" in {
        val flightWithSplits = flightWithPaxAndApiSplits(None, 0, 100, 0, Set(LiveFeedSource), SortedSet(TotalPaxSource(40, LiveFeedSource, None)))
        flightWithSplits.hasValidApi mustEqual true
    ...
    "give a pax count from splits when it has API splits" in {
      val flightWithSplits = flightWithPaxAndApiSplits(Option(40), 0, 45, 0, Set(), SortedSet())
    ...
    "give a pax count from splits when it has API splits which does not include transfer pax" in {
      val flightWithSplits = flightWithPaxAndApiSplits(Option(40), 0, 45, 20, Set(), SortedSet())
      flightWithSplits.totalPaxFromApiExcludingTransfer.map(_.pax) mustEqual Option(45)
    }

    "give a pax count from splits when it has API splits even when it is outside the trusted threshold" in {
      val flightWithSplits = flightWithPaxAndApiSplits(Option(40), 0, 150, 0, Set(LiveFeedSource), SortedSet())
  ...
  private def flightWithPaxAndApiSplits(actPax: Option[Int], transferPax: Int, splitsDirect: Int, splitsTransfer: Int, sources: Set[FeedSource], totalPax: SortedSet[TotalPaxSource]): ApiFlightWithSplits = {
