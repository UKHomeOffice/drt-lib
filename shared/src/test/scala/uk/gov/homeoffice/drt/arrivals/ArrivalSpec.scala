...
class ArrivalSpec extends Specification {
  "An Arrival" should {
    ...
    "Know it has no source of passengers when there are no sources with a pax figure" in {
      ArrivalGeneratorShared.arrival(passengerSources =
        Map(LiveFeedSource -> Passengers(None, None))
      ).hasNoPaxSource shouldEqual true
    }
    "Know it has a source of passengers when there is a source with a pax figure" in {
      ArrivalGeneratorShared.arrival(passengerSources =
        Map(LiveFeedSource -> Passengers(Option(100), None))
      ).hasNoPaxSource shouldEqual false
    }
  }
  ...
  "Arrival bestPcpPaxEstimate" should {
    ...
    "Give Forecast feed source as bestPcpPaxEstimate, " +
      "When Forecast feed source and HistoricApiFeedSource is present in total pax" in {
        ...
        arrival.bestPaxEstimate(sourceOrderPreference) mustEqual
          PaxSource(portForecastFeedPaxSource._1, portForecastFeedPaxSource._2)
      }
    ...
    "When totalPax " +
      ...
      " bestPcpPaxEstimate gives Live feed source" in {
        val arrival = arrivalBase.copy(PassengerSources =
          Map(
            liveFeedPaxSource,
    ...
    "When totalPax" +
      ...
      " bestPcpPaxEstimate gives api with historic feed" in {
        ...
        arrival.bestPaxEstimate(sourceOrderPreference) mustEqual
          PaxSource(historicApiFeedPaxSource._1, historicApiFeedPaxSource._2)
      }
    ...
    "When totalPax" +
      ...
      " then bestPcpPaxEstimate gives LiveFeedSource with zero pax " in {
        val arrival = arrivalBase.copy(
          PassengerSources =
            Map(AclFeedSource -> Passengers(Option(250), None), LiveFeedSource -> Passengers(Option(50), Option(100)))
        )
        arrival.bestPaxEstimate(sourceOrderPreference) mustEqual
          PaxSource(LiveFeedSource, Passengers(Option(50), Option(100)))
    ...
    "Give zero pcp when the flight is domestic" >> {
      val arrival = arrivalBase.copy(
        Origin = PortCode("LHR"),
        PassengerSources = Map(LiveFeedSource -> Passengers(Option(10), Option(3)))
      )
      arrival.bestPcpPaxEstimate(sourceOrderPreference) must beSome(0)
    }
  ...
  "minutesOfPaxArrivals" >> {
    "should return 0 when there are 0 total passengers" >> {
      ArrivalGeneratorShared.arrival(passengerSources =
        Map(ApiFeedSource -> Passengers(Option(0), None))
      ).minutesOfPaxArrivals(List(ApiFeedSource)) === 0
    }
    "should return 1 when there are 1 total passengers" >> {
      ArrivalGeneratorShared.arrival(passengerSources =
        Map(ApiFeedSource -> Passengers(Option(1), None))
      ).minutesOfPaxArrivals(List(ApiFeedSource)) === 1
    }
    "should return 1 when there are 20 total passengers" >> {
      ArrivalGeneratorShared.arrival(passengerSources =
        Map(ApiFeedSource -> Passengers(Option(20), None))
      ).minutesOfPaxArrivals(List(ApiFeedSource)) === 1
    }
    "should return 2 when there are 21 total passengers" >> {
      ArrivalGeneratorShared.arrival(passengerSources =
        Map(ApiFeedSource -> Passengers(Option(21), None))
      ).minutesOfPaxArrivals(List(ApiFeedSource)) === 2
    }
  }

  "pcpRange" >> {
    "should return 0L to 0L given an arrival with scheduled time 0L and 0 total passengers" >> {
      ArrivalGeneratorShared.arrival(
        sch = 0L,
        passengerSources = Map(ApiFeedSource -> Passengers(Option(0), None))
      ).pcpRange(List(ApiFeedSource)) === (0L to 0L by 60000)
    }
    "should return 0L to 0L given an arrival with scheduled time 0L and 1 total passengers" >> {
      ArrivalGeneratorShared.arrival(
        sch = 0L,
        passengerSources = Map(ApiFeedSource -> Passengers(Option(1), None))
      ).pcpRange(List(ApiFeedSource)) === (0L to 0L by 60000)
    }
    "should return 0L to 0L given an arrival with scheduled time 0L and 20 total passengers" >> {
      ArrivalGeneratorShared.arrival(
        sch = 0L,
        passengerSources = Map(ApiFeedSource -> Passengers(Option(20), None))
      ).pcpRange(List(ApiFeedSource)) === (0L to 0L by 60000)
    }
    "should return 0L to 60000L given an arrival with scheduled time 0L and 21 total passengers" >> {
      ArrivalGeneratorShared.arrival(
        sch = 0L,
        passengerSources = Map(ApiFeedSource -> Passengers(Option(21), None))
      ).pcpRange(List(ApiFeedSource)) === (0L to 60000L by 60000)
    }
  }
