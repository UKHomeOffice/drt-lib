...
case class ApiFlightWithSplits(apiFlight: Arrival, splits: Set[Splits], lastUpdated: Option[Long] = None)
  ...
  def bestSplits: Option[Splits] = {
    val apiSplitsDc = splits.find(s =>
      s.source == SplitSources.ApiSplitsWithHistoricalEGateAndFTPercentages &&
        (s.totalPax == 0 || s.totalPax != s.transPax)
    )
    val scenarioSplits = splits.find(s =>
      s.source == SplitSources.ScenarioSimulationSplits && (s.totalPax == 0 || s.totalPax != s.transPax)
    )
    val historicalSplits =
      splits.find(s => s.source == SplitSources.Historical && (s.totalPax == 0 || s.totalPax != s.transPax))
  ...
  def hasValidApi: Boolean = {
    ...
    (maybeApiSplits, hasLiveSource, hasSimulationSource) match {
      ...
      case (Some(api), true, _)
          if api.isWithinThreshold(apiFlight.PassengerSources.get(LiveFeedSource), liveApiTolerance) => true
      case _ => false
    }
  }
