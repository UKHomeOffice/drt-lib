...
case class PaxCapModelAndFeaturesV2(
    ...
    val updatedPassengers = maybePrediction(arrival, minimumImprovementPctThreshold, upperThreshold) match {
      ...
      case Some(pctFull) =>
        val pax = arrival.MaxPax
          ...
          .getOrElse {
            scribe.warn(
              s"Unknown capacity for ${arrival.flightCode} @ ${arrival.Scheduled}. Using fallback value of $fallback"
            )
            fallback
          }
