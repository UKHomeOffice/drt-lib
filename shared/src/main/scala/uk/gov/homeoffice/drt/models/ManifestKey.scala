...
case class ManifestKey(
    origin: PortCode,
    voyageNumber: VoyageNumber,
    scheduled: Long
) extends Ordered[ManifestKey] with WithTimeAccessor {
...
object ManifestKey {
  ...
  def apply(arrival: Arrival): ManifestKey =
    ManifestKey(arrival.PreviousPort.getOrElse(arrival.Origin), arrival.VoyageNumber, arrival.Scheduled)

  def atTime: MillisSinceEpoch => ManifestKey =
    (time: MillisSinceEpoch) => ManifestKey(PortCode(""), VoyageNumber(0), time)
}
