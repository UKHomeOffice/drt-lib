    ...
    queuesByTerminal.foreach {
      case (date, config) => config.foreach {
          case (terminal, tQueues) =>
            assert(
              minMaxDesksByTerminalQueue24Hrs.contains(terminal),
              s"Missing min/max desks: ${date.toISOString} $terminal @ $portCode"
            )
            tQueues
              .filterNot(_ == Transfer)
              .foreach {
                tQueue =>
                  assert(
                    minMaxDesksByTerminalQueue24Hrs(terminal).contains(tQueue),
                    s"Missing min/max desks: ${date.toISOString} $tQueue for terminal $terminal @ $portCode"
                  )
              }
        }
    }
      ...
      case (terminal, procTimes) =>
        procTimes.foreach {
          case (paxTypeAndQueue, procTime) =>
            assert(
              procTime > 0.3,
              s"Processing time for $paxTypeAndQueue at $terminal is too low @ $portCode (${procTime * 60} <= 20s)"
            )
            assert(
              procTime < 3,
              s"Processing time for $paxTypeAndQueue at $terminal is too high @ $portCode (${procTime * 60} >= 180s)"
            )
        }
    }
  }

  def minDesksByTerminalAndQueue24Hrs: Map[Terminal, Map[Queue, IndexedSeq[Int]]] =
    minMaxDesksByTerminalQueue24Hrs.mapValues(_.mapValues(_._1.toIndexedSeq).view.toMap).view.toMap

  def maxDesksByTerminalAndQueue24Hrs: Map[Terminal, Map[Queue, IndexedSeq[Int]]] =
    minMaxDesksByTerminalQueue24Hrs.mapValues(_.mapValues(_._2.toIndexedSeq).view.toMap).view.toMap

  def minDesksForTerminal24Hrs(tn: Terminal): Map[Queue, IndexedSeq[Int]] =
    minMaxDesksByTerminalQueue24Hrs.getOrElse(tn, Map()).mapValues(_._1.toIndexedSeq).view.toMap

  def maxDesksForTerminal24Hrs(tn: Terminal): Map[Queue, IndexedSeq[Int]] =
    minMaxDesksByTerminalQueue24Hrs.getOrElse(tn, Map()).mapValues(_._2.toIndexedSeq).view.toMap

  val terminalsForDate: LocalDate => Iterable[Terminal] = QueueConfig.terminalsForDate(queuesByTerminal)

  val terminalsForDateRange: (LocalDate, LocalDate) => Iterable[Terminal] =
    QueueConfig.terminalsForDateRange(queuesByTerminal)
...
object AirportConfig {
  implicit val rwQueues: ReadWriter[SortedMap[Terminal, Seq[Queue]]] =
    readwriter[Map[Terminal, Seq[Queue]]].bimap[SortedMap[Terminal, Seq[Queue]]](
