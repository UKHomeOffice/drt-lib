...
case class Arrival(
  ...
  val isCancelledStatus: String => Boolean = description =>
    description == "c" | description == "canceled" | description == "deleted / removed flight record" |
      description == "cancelled" | description.contains("deleted")
  ...
  lazy val minutesToChox: Int =
    Predictions.predictions.getOrElse(ToChoxModelAndFeatures.targetName, Arrival.defaultMinutesToChox)
  ...
  override def update(incoming: Arrival): Arrival =
    incoming.copy(
      BaggageReclaimId =
        if (incoming.BaggageReclaimId.exists(_.nonEmpty)) incoming.BaggageReclaimId else this.BaggageReclaimId,
...
object Arrival {
  ...
  def isRelevantToPeriod(
      rangeStart: SDateLike,
      rangeEnd: SDateLike,
  ...
  def summaryString(arrival: Arrival): String = arrival.AirportID + "/" + arrival.Terminal + "@" + arrival.Scheduled +
