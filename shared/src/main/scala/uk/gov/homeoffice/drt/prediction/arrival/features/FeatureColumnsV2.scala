...
object FeatureColumnsV2 {
  ...
  case object BestPax extends SingleFeature[Arrival] {
    ...
    override val value: Arrival => Option[Double] =
      _.bestPaxEstimate(List(ApiFeedSource, LiveFeedSource)).passengers.getPcpPax.map(_.toDouble)
  }

  object OneToMany {
    def fromLabel(label: String)(implicit
  ...
  case class PostPandemicRecovery(recoveryDate: SDateLike)(implicit sDateProvider: Long => SDateLike)
      extends OneToManyFeature[Arrival] {
  ...
  case class OctoberHalfTerm()(implicit
      val sDateTs: Long => SDateLike,
      val sDateLocalDate: LocalDate => SDateLike
  ) extends OneToManyFeature[Arrival] with HolidayLike {
  ...
  case class ChristmasHoliday()(implicit
      val sDateTs: Long => SDateLike,
      val sDateLocalDate: LocalDate => SDateLike
  ) extends OneToManyFeature[Arrival] with HolidayLike {
  ...
  case class SpringHalfTerm()(implicit
      val sDateTs: Long => SDateLike,
      val sDateLocalDate: LocalDate => SDateLike
  ) extends OneToManyFeature[Arrival] with HolidayLike {
  ...
  case class EasterHoliday()(implicit
      val sDateTs: Long => SDateLike,
      val sDateLocalDate: LocalDate => SDateLike
  ) extends OneToManyFeature[Arrival] with HolidayLike {
  ...
  case class SummerHalfTerm()(implicit
      val sDateTs: Long => SDateLike,
      val sDateLocalDate: LocalDate => SDateLike
  ) extends OneToManyFeature[Arrival] with HolidayLike {
  ...
  case class SummerHolidayScotland()(implicit
      val sDateTs: Long => SDateLike,
      val sDateLocalDate: LocalDate => SDateLike
  ) extends OneToManyFeature[Arrival] with HolidayLike {
  ...
  case class SummerHoliday()(implicit
      val sDateTs: Long => SDateLike,
      val sDateLocalDate: LocalDate => SDateLike
  ) extends OneToManyFeature[Arrival] with HolidayLike {
  ...
  trait HolidayLike {
    ...
    def localDateRange(start: LocalDate, end: LocalDate)(implicit sdate: LocalDate => SDateLike): Seq[LocalDate] = {
      @tailrec
    ...
    def rangeBaseValues(length: Int, base: Int): IndexedSeq[Int] = (0 until length)
      .map(d => 100 * d / length)
  ...
  case object FlightNumber extends OneToManyFeature[Arrival] {
    ...
    override val value: Arrival => Option[String] =
      (a: Arrival) => Option(a.flightCode.voyageNumberLike.numeric.toString)
  }
}
