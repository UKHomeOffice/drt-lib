...
object ArrivalLike {
  implicit val rw: ReadWriter[ArrivalLike] = ReadWriter.merge(Arrival.rw, ForecastArrival.rw)
}
...
trait ArrivalLike {
  ...
  def update(incoming: ArrivalLike): ArrivalLike
  ...
  def withoutPcpTime: ArrivalLike

  def isEqualTo(arrival: ArrivalLike): Boolean =
